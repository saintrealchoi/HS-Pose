...
def save_projected_points(color_img, pcd_array, output_path, uid, pred_class):
    open_3d_lines = [
        [5, 3],
        [6, 4],
        [0, 2],
    ...
    for i, points_2d_mesh in enumerate(pcd_array):
        ...
        plt.scatter(points_2d_mesh[:,0], points_2d_mesh[:,1], color=color[class_id], s=2)
    plt.gca().invert_yaxis()
    plt.axis('off')
    plt.imshow(color_img[:,:,::-1])
...
def draw_2d_bboxes(img, mrcnn):
    colormap = [(0,0,255),(0,255,0),(255,0,0),(255,255,0),(0,255,255),(255,0,255)]
    for i in range(len(mrcnn['gt_class_ids'])):
...
def draw_bboxes(img, img_pts, axes, color):
    ...
    # draw axes
    img = cv2.arrowedLine(img, tuple(axes[0]), tuple(axes[1]), (0, 0, 255), 3)
    img = cv2.arrowedLine(img, tuple(axes[0]), tuple(axes[3]), (255, 0, 0), 3)
    img = cv2.arrowedLine(img, tuple(axes[0]), tuple(axes[2]), (0, 255, 0), 3) ## y last

    return img

def draw_axes(img, img_pts, axes, color):
    # draw axes
    img = cv2.arrowedLine(img, tuple(axes[0]), tuple(axes[1]), (250,218,94), 3)
    img = cv2.arrowedLine(img, tuple(axes[0]), tuple(axes[3]), (146,52,182), 3)
    img = cv2.arrowedLine(img, tuple(axes[0]), tuple(axes[2]), (28,28,28), 3) ## y last
