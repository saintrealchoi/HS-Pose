...
class FaceRecon(nn.Module):
    def __init__(self):
        ...
        self.conv_0 = gcn3d.HSlayer_surface(kernel_num=128, support_num=self.support_num)
        # self.conv_0 = gcn3d.HS_layer(6, 128, support_num=self.support_num)
    ...
    def forward(self,
        ...
        gray_mean = torch.mean(bgr,dim=1,keepdim=True)
        gray_std = torch.std(bgr,dim=1,keepdim=True)
        norm_gray = (bgr-gray_mean) / gray_std
        vertices = torch.cat((vertices,norm_gray),dim=2)
        ...
        fm_1 = F.relu(self.bn1(self.conv_1(vertices[:,:,:3], fm_0, self.neighbor_num).transpose(1, 2)).transpose(1, 2), inplace=True)
        v_pool_1, fm_pool_1 = self.pool_1(vertices[:,:,:3], fm_1)
        ...
        nearest_pool_1 = gcn3d.get_nearest_index(vertices[:,:,:3], v_pool_1)
        nearest_pool_2 = gcn3d.get_nearest_index(vertices[:,:,:3], v_pool_2)
        ...
        if FLAGS.train:
            ...
            feat_face_in = torch.cat([feat_face_re, conv1d_out, vertices[:,:,:3].permute(0, 2, 1)], dim=1)
