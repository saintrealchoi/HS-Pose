...
class DANet(nn.Module):
	def __init__(self):
		...
		self.feature_extraction = get_models()
		self.max_depth=FLAGS.max_depth
		self.min_depth=FLAGS.min_depth
		if FLAGS.backbone in ["MobileNetV2"]:
			block_channel = [16, 24, 32, 96, 320]

		if FLAGS.backbone in ["EfficientNet"]:
			block_channel = [16, 24, 40, 112, 320]

		if FLAGS.backbone in ["ResNet18", "ResNet34"]:
			block_channel = [64, 64, 128, 256, 512]

		if FLAGS.backbone in ["ResNet50", "ResNet101", "ResNet152"]:
